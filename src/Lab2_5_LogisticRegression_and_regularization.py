...
class LogisticRegressor:
    ...
    def fit(self,X,y,
        ...
        # print(f"las dimensiones de x son {m,n}")
        # print(f"Las dimensiones de y son {y.shape}")

        # TODO: Initialize all parameters to 0

        self.weights = np.zeros(n) #los pesos seran una matriz de todo ceros con un filas osea tantas filas como carcateristics haya, asi
        # para cda observacion que tenga x se ira multilplicando cad acracatreristica por su peso correspondiente
        self.bias = 0
        ...
        for i in range(num_iterations):  # Fill the None here
            ...
            # print(f"Dimensión de (y_hat - y): {(y_hat - y).shape}")

            dw = (1/m) * np.dot(X.T, (y_hat - y)) #es

            # print(f"Dimensión de dw: {dw.shape}")

            db = (1/m) * np.sum(y_hat - y) #esto tendra las mismas dimensiones que y, osea tantas filas como observaciones
            ...
            # Update parameters
            # print(f"los pesos son {self.weights} y dw es {dw}")
            self.weights -= learning_rate * dw
            self.bias -= learning_rate * db

            # print(self.weights.shape, X.shape[1])

            # model.weights.shape == (X_train.shape[1],)
    ...
    def predict_proba(self, X):
        ...
        # # p=1/(1+np.exp(-(self.bias+X*self.weights)))
        z = self.bias+np.dot(X, self.weights)
        #z es el producto de las observaciones x con los pesos 
    ...
    def sigmoid(z):
        ...
        #lo que se devuelve es el valor de una probbailidad, la funcion sigmoide coge un x y devuelve la probabilidad (rango 0-1)
        #devuelve la probabilidad de que una observacion o dato x pertenezca a la clase j
